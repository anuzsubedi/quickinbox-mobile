...
struct SettingsView: View {
    ...
    private var connectionPage: some View {
        ...
        .alert("Disconnect This Device?", isPresented: $showingDisconnectConfirmation) {
            ...
            Text("QuickInbox will revoke this session and remove its saved account and mailbox data from this device.")
        }
        .alert("Remove Local Data?", isPresented: $showingLocalWipeConfirmation) {
            ...
            Text("QuickInbox will remove its saved account and mailbox data without contacting your server. This device may still need to be revoked on the web.")
        }
    }
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if dynamicTypeSize.isAccessibilitySize, let detail {
                    Text(detail)
                        .font(.quickInboxBody(15, relativeTo: .subheadline))
                        .foregroundStyle(QuickInboxDesign.Palette.secondaryText)
                }
            }
    ...
    private var accountSection: some View {
        settingsPageSection(
            "Details",
            detail: "This app stays connected directly to your QuickInbox server."
        ) {
            ...
            if let manageURL = model.manageWebURL {
                ...
                .accessibilityHint("Opens your QuickInbox account settings in the browser")
            }
        }
    }
    ...
    private var serverSection: some View {
        settingsPageSection(
            "Connection",
            detail: "This device connects directly to your QuickInbox server over HTTPS."
        ) {
            if let serverName {
                HStack(spacing: 14) {
                    Image(systemName: "server.rack")
                        .font(.body.weight(.medium))
                        .foregroundStyle(QuickInboxDesign.Palette.signalInk)
                        .frame(width: 28)
    ...
    private var sendingAddressSection: some View {
        settingsPageSection(
            ...
            detail: model.addresses.count > 1
                ? "QuickInbox preselects this address when you start a new message on this device."
    ...
    private func deviceIdentity(_ device: DeviceSession) -> some View {
        HStack(spacing: 12) {
            Image(systemName: model.symbolName(for: device))
                .font(.body.weight(.medium))
                .foregroundStyle(device.isCurrent ? QuickInboxDesign.Palette.sage : QuickInboxDesign.Palette.secondaryText)
    ...
    private func disconnectButtonLabel(_ title: String) -> some View {
        Group {
            if model.isDisconnecting {
                ...
                Text(title)
                    .font(.quickInboxSemibold(15, relativeTo: .subheadline))
                    .frame(minHeight: 32)
            }
        }
    }
    ...
    private var privacySection: some View {
        settingsPageSection(
            "Privacy",
            detail: "When enabled, QuickInbox hides message content after you leave the app and asks for \(appLock.biometryName) or your device passcode when you return."
        ) {
            Toggle(
                isOn: Binding(
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("App Lock")
                    Text("Require authentication when returning to QuickInbox")
                        .font(.quickInboxBody(13, relativeTo: .caption))
                        .foregroundStyle(QuickInboxDesign.Palette.secondaryText)
                }
            }
...
private final class SettingsViewModel: ObservableObject {
    ...
    init(api: QuickInboxAPI, currentUser: User) {
        self.api = api
    ...
    func disconnect(revokeOnServer: Bool) async -> String? {
        ...
        if revokeOnServer, revocationError != nil {
            return "This iPhone was disconnected locally, but the server could not confirm revocation. Revoke it from QuickInbox on the web."
        }
        return nil
    }
